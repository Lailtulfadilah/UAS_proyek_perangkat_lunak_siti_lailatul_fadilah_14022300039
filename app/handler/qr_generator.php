...
// Function to update the code in qr_code table - FIXED VERSION
function updateCode($pdo, $code) {
    // First, get the latest ID
    $stmt = $pdo->query("SELECT id FROM qr_code ORDER BY id DESC LIMIT 1");
    $id = $stmt->fetchColumn();
    
    // Then update using the retrieved ID
    $stmt = $pdo->prepare("UPDATE qr_code SET kode_unik = :kode_unik WHERE id = :id");
    $stmt->execute([
        'kode_unik' => $code,
        'id' => $id
    ]);
